<?php
	...
	#########################################################
		...
		$menu = ($_GET['menu'])?$_GET['menu']:"untreated";
		...
		$sql_common_menu=''; // FROM 구문.
		$sql_search_menu=''; // WHERE 절에 조건을 검. - article, comment, 등.
		$sql_group_menu=''; //WHERE절 다음에 걸릴 GROUP BY를 걸기 위한 메뉴 -> blinded_list 테이블 추가하면서 group by cnt>5가 subquery 밖으로 나옴.
		switch($menu) {
			case 'untreated':
				$menu_attr = (!$menu_attr)?"all":$menu_attr; //미처리 신고내역의 경우 "all을 기본값으로"
				$menu_title = "전체 미처리 신고 내역";
				$sql_common_menu		.= " FROM `".$piree_table['report_list']."` " ;
				
				//$use_where = 1;
				break;
			case 'blinded':
				//$sql_search_menu		.= " AND prog_div_c='".$menu."' ";;
				$menu_title = "블라인드된 게시물 신고 내역";
				
				$write_table = $g5['write_prefix'].$report_row["bo_table"];
				$sql_ = " ( SELECT * FROM `{$write_table}` WHERE wr_blind = 1 ) ";
				
				//============== 2016-07-28 by Bryan Park =============
				// 방식이 -> 블라인드 상태인 게시물들을 등록해두는 blinded_list table에서 가져오는 것으로 변경.
				// 그래서 FROM 문에서 blinded_list를 가져오도록.
				$sql_common_menu		.= " FROM `".$piree_table['report_list']."` ";
				//WHERE절에 넣을 sql_search : menu
				$sql_search_menu	.= " AND ";
				$sql_search_menu	.= "(";
				$sql_search_menu	.=	"(bo_table, arti_n) IN ";
				$sql_search_menu	.=	"(SELECT bo_table,wr_id FROM `".$piree_table['blinded_list']."` )";
				$sql_search_menu	.= ")";
				$sql_group_menu		.= " GROUP BY bo_table, arti_n ";
				/*
				// //2016-07-27 기존의 group by cnt subquery를 사용하기 위한 sql문 정의 부분. 이 부분이 위에 작성할 sql 정의로 대체되어야 함. by Bryan Park
				...
				//2016-07-27 where 절에 넣기 위한 부분 추가. report count 이상으로 걸린 것. by Bryan Park $rept_cfg__blind_do_t <- index.php에 정의됨.
				$sql_search_menu= " AND report_group.cnt >= $rept_cfg__blind_do_t ";
				//$sql_common_menu		.=  "WHERE report_group.cnt >= 3 ";
				*/
				
				break;
			case 'article':
				$menu_title = "게시물에 대한 신고 내역";
				$sql_search_menu		.= " AND prog_div_c='".$menu."' ";;

				$sql_common_menu		.= "FROM `".$piree_table['report_list']."` ";
				break;
			case 'comment':
				$menu_title = "댓글에 대한 신고 내역";
				$sql_search_menu		.= " AND prog_div_c='".$menu."' ";;
		...
		$sql_search = " where (1) ";
		if($use_where){//2016-07-26 untreated일때나 다른 탭에서 미처리만 보기시 사용. by Bryan Park
		$sql_search	.=	"	and (active_s='".$report_active_s__new."' OR  active_s = '보류' ) ";
		}
		//2016-07-26 각 탭별 조건 추가. by Bryan Park
		$sql_search .= $sql_search_menu;
		...
		//echo $sql.": TEST - 쿼리문 확인 <br>- line 813 of list.included.php";
		//TEST
		...
		// 리스트_URL
		$qstr .= "&bid={$bid}&bno={$bno}&menu={$menu}&mattr={$menu_attr}";
		...
		<div style="margin:0 20px 0 20px; border:3px solid #cfcfcf; padding:15px;">
			...
			<div class="tbl_frm01">
				...
				<tr>
						...
						<td>
							<div class="report_box">
								<div class="float_left_600"><span class="font_777">구분 : </span>	<strong style="color:blue"><?php echo $report__prog_s; ?>
								...
								<hr style ="display:block;border-style:solid;border-width: 0.5px; color:#aaaaaa">
								<div class="float_left_600" ><span class="font_777">내용 : </span>
								<?php echo $report_cont_s; ?>
								</div>
								<hr style ="display:block;border-style:solid;border-width: 0.5px; color:#aaaaaa">
				...
				<tr>
						...
						//============== 2016-07-27 by Bryan Park =============
						// 블라인드 된 게시물의 sql_common ( FROM 절 ) 을 일반 형태로 변경.
						$sql_or_common = " FROM ".$piree_table['report_list']." ";
						// 현재 선택중인 메뉴에 맞는 조건 부여.( ex) article, comment, member...)
						$menu_or = ($menu != 'blinded' && $menu != 'untreated')?$menu : 'article'; // 기본값은 article로.
						...
						$sql_or_search .= " AND (`active_s` = '접수' OR `active_s` = '보류' ) "; // 미처리된 신고건만 [`res_mem_mn` 처리한 관리자의 회원 no -> 0일때는 미처리된 신고건이라는 뜻.] => 접수 또는 보류 상태인 것을 직접 확인하게 수정.
						$sql_or_search .= " AND (`prog_div_c` = '{$menu_or}' ) "; ////2016-07-27 by Bryan Park게시물, 코맨트, 등의 구분.
						$sql_or_search .= ")";
						$sql_other_reports = "SELECT * {$sql_or_common} {$sql_or_search} ";
						...
						<td>
							...
							//시작 => 신고받은 횟수 출력
								...
								echo "[총 ".$report_count_total."회] ".$report_count_n_reason_str;
						...
						<td>
							...
							//시작 =>  동일 게시물에 대한 미처리 내역 출력.
								...
								while($row_or = sql_fetch_array($result_other_reports)){
									...
									//2016-07-27 $in_link_s by Bryan Park
									$in_link_s = "index.php?{$qstr}&report_n=".$row_or['num'];
									//echo $in_link_s."<-in_link_s<br>";
									echo "<a style='color:red;'href='{$in_link_s}'>".
...
?>
								...
								<div class="float_left_600">
									<div class="btn_confirm">
										<input type="submit" class="btn_submit" value=" 실행하기 ">
									</div>						
								</div>
							</div>
...
<section>
		<h2 style="display:inline-block; margin: 0px;"><?=$menu_title?>
		<?if($menu!='untreated1'){?>
			<input type="button" class="btn_cancel tab_btn<?=set_btn_active($menu_attr, "all")?>"  value="전체" onClick="PageUrl('./?menu=<?=$menu?>&mattr=all')">
			<input type="button" class="btn_cancel tab_btn<?=set_btn_active($menu_attr,"unprocessed")?>"  value="미처리" onClick="PageUrl('./?menu=<?=$menu?>&mattr=unprocessed')">
		<?}?>
		</h2>
		
		<form id="fsearch" name="fsearch" class="local_sch01 local_sch" method="get" action="index.php?$qstr">
			<label for="sfl" class="sound_only">검색대상</label>
			<input type="hidden" class="menu_value" name="menu" id="menu" value="<?=$menu?>">
			<input type="hidden" class="menu_value" name="mattr" id="mattr" value="<?=$menu_attr?>">
			...
			<?php if($sfl||$bid||$bno) {?>
			<input type="button" class="btn_submit" value="검색 지우기" onClick="PageUrl('./index.php?menu=<?=$menu?>&mattr=<?=$menu_attr?>')">
			<?}?>
		</form>
...
<?php
		...
		IF ($total_count > 0)
		{
			...
			FOR ($i=0; $row=sql_fetch_array($result); $i++)
			{
				...
				$in_link_s = "index.php?{$qstr}&report_n=".$row['num'];
...
?>
				<tr>
					...
					<td>
						...
						?>
						<br />
						<?if($menu!='blinded'){ //2016-07-29 메뉴가 블라인드 게시물일땐 중복없이 게시물 단위로 나오기에 필요없음. by Bryan Park?>
...
?>
						<br />
						<span class="font_777">메모</span> 
						<a href="<?php echo $in_link_s ?>"<?php echo $in_style_s ?>>
							<?php echo $in_arrow_s ?><?php echo $in_report_memo_s ?>
						</a>
<?php
