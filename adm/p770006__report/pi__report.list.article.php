...
#########################################################
	...
	$total = sql_efv($sql);
	*/
	//2016-07-19 게시물에 대한 신고만 가져오게끔 해야함. by Bryan Park
	$sql	 = "SELECT COUNT(*) FROM `".$piree_table['report_list']."` ";
	...
	IF ($total > 0)
	{
		...
		*/
		...
		$sql	 = "SELECT * FROM `".$piree_table['report_list']."` ";
		$sql	.= "";
		//$prog_div_c == article -> article에 대한 신고만 가져오게끔.
		$sql	.= "WHERE prog_div_c='".$prog_div_c."' ";
		if($use_where){
		$sql	.= " AND active_s='".$report_active_s__new."' ";
		}
...
<section>
		<h2 id="blinded">
		게시물 신고내역 (개발중 )<?=$sql?>
			<input type="button" class="btn_cancel tab_btn<?=set_btn_active($menu_attr, "all")?>"  value="전체" onClick="PageUrl('./?menu=article&mattr=all#blinded')">
			<input type="button" class="btn_cancel tab_btn<?=set_btn_active($menu_attr,"unprocessed")?>"  value="미처리" onClick="PageUrl('./?menu=article&mattr=unprocessed#blinded')">
		</h2>

		<div class="tbl_head01 tbl_wrap">
				<table>
				<caption>게시물 신고내역</caption>
				<thead>
				<tr>
					<th scope="col">번호</th>
