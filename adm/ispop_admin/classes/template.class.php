...
class Template extends Template_
{
	...
	public function __construct($skin='') {
		global $default,$config,$member,$group,$g5,$mb,$ispop, $eb;
		if($skin) $this->skin = $skin; else $this->skin = $this->def_skin;
		$this->eb = $eb;

		// 템플릿 기본설정
		$this->template_dir	= ISPOP_PATH."/theme";
		...
		$this->define(array(
			...
			'custom_page_head_pc' => 'layout/custom_page_head_pc.html',
			'custom_page_head_mo' => 'layout/custom_page_head_mo.html',
		));
	...
	// 영카트5를 이윰파일로 컨트롤 하기
	public function ispop_control() {
		$path = $this->get_filename_from_url();
		$file = ISPOP_SHOP_PATH.'/'.$path['filename'];
	...
	public function exchange_file() {
		...
		if($this->pattern_match_url("/bbs\/new_delete/i")) {
			return ISPOP_CORE_PATH.'/new/'.$path['filename'];
		}
		// 쪽지보내기 기능 - File Hooking
		if($this->pattern_match_url("/bbs\/memo_form_update/i")) {
			return ISPOP_CORE_PATH.'/member/'.$path['filename'];
		}
		...
		if($this->pattern_match_url("/bbs\/poll_result/i") && $ispop['use_gnu_poll'] == 'n') {
			return ISPOP_CORE_PATH.'/poll/'.$path['filename'];
		}
	}
