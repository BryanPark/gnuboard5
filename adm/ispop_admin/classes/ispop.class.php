...
class ispop extends qfile
{
	...
	public function print_page($target) {
		global $g5, $tpl, $tpl_name, $ispop, $member, $user, $ispoper, $eb, $config, $levelset, $is_admin, $is_member;
			...
			if($index || $dummy_id == 'home' || $dummy_id == 'auto_login' || $dummy_id == 'device') {
				...
				if(exist_mb_id($dummy_id)) {
					...
					if($user['open_page']=='y' || ($user['mb_id'] == $member['mb_id'] && $user['mb_id']) ) {
						...
						$tpl->define_template('mypage',$ispop['mypage_skin'],'myhome.skin.html');
						$tpl_index = $tpl_name;
					} else {
	...
	// 이윰보드 설정정보
	public function ispop_board_info($bo_table, $theme) {
		global $g5;
		$sql = "select a.*,b.bo_subject,c.gr_subject from {$g5['ispop_board']} as a left join {$g5['board_table']} as b on a.bo_table = b.bo_table left join {$g5['group_table']} as c on b.gr_id = c.gr_id where a.bo_table='{$bo_table}' and a.bo_theme='{$theme}'";
	...
	public function respond($respond = array()) {
		...
		$row = sql_fetch(" select rid from {$g5['ispop_respond']} where $where and re_chk <> '1' order by rid desc ", false);
		$rid = $row['rid'];

		if($rid) {
			// 열람하지 않은 내글반응이 이미 있을 경우, 카운트만 올림
			sql_query("update {$g5['ispop_respond']} set re_cnt=re_cnt+1, regdt='".G5_TIME_YMDHIS."' where rid='{$rid}'", false);
		} else {
			// 내글 반응 등록
			$insert = " insert into {$g5['ispop_respond']} set $set regdt = '".G5_TIME_YMDHIS."' ";
			sql_query($insert, false);
			$rid = mysql_insert_id();

			// 원본글 작성자의 반응글 적용
			$row = sql_fetch("select mb_id from {$g5['ispop_member']} where mb_id = '{$wr_mb_id}'", false);
			if($row['mb_id']) {
				sql_query(" update {$g5['ispop_member']} set respond = respond + 1 where mb_id = '{$wr_mb_id}' ", false);
			} else {
				sql_query(" insert into {$g5['ispop_member']} set mb_id = '{$wr_mb_id}', respond=1", false);
			}
		}

		// 푸시등록
		$user = sql_fetch("select onoff_push_respond from {$g5['ispop_member']} where mb_id = '{$wr_mb_id}'");
	...
	// 호스트명 추출
	public function ispop_host($url='') {
		if(!$url) $url = G5_URL;
		$info = parse_url($url);
	...
	public function insert_activity($mb_id, $type, $content) {
		...
		$sql = "
			insert into {$g5['ispop_activity']} set 
				mb_id = '{$mb_id}',
				act_type = '{$type}',
	...
	public function get_user_info($mb_id='') {
		...
		//echo "<script>alert('get_user_info_has_called - {$mb_id}');</script>";
		...
		$sql_common = " from {$g5['member_table']} as a	left join {$g5['ispop_member']} as b on a.mb_id = b.mb_id ";
		...
		if($single) {
			...
			if($user['mb_id']) {
				...
				$insert = "insert into {$g5['ispop_member']} set mb_id = '{$mb_id}'";
				sql_query($insert, false);
	...
	public function level_point($point,$r_id='',$r_point=0) {
		global $g5, $ispoper, $is_admin;
		print_r($ispoper);
		if($point) {
			$level_point = $ispoper['level_point'];
			$point_sum = $level_point + $point;
			
			$level = $this->get_level_from_point($point_sum,$ispoper['level']);
			$sql = "update {$g5['ispop_member']} set level='{$level}', level_point='{$point_sum}' where mb_id='{$ispoper['mb_id']}'";
			sql_query($sql, false);

			if($r_id) {
				$sql = "update {$g5['ispop_member']} set level_point=level_point+".$r_point." where mb_id='{$r_id}'";
				sql_query($sql, false);
			}
		} else return false;
	}
	...
	public function set_gnu_level($level) {
		global $g5, $member;
		$mb_level = $this->get_gnulevel_from_ispoplevel($level);
	...
	// 이윰레벨에서 그누레벨 가져오기
	private function get_gnulevel_from_ispoplevel($level) {
		global $levelset;
		$gnulevel = array();
	...
	public function ispop_level_info($member) {
		global $ispoper, $levelinfo, $levelset;
		$lvinfo = $levelinfo[$ispoper['level']];
		$bar_len = $lvinfo['max'] - $lvinfo['min'];
		$lv_len = $ispoper['level_point'] - $lvinfo['min'];
		$ratio = ($lv_len/$bar_len)*100;
		if($ratio >= 100) {
			$ispoper['level'] = $ispoper['level']+1;
			$this->level_point(1);
			$lvinfo = $levelinfo[$ispoper['level']];
			$bar_len = $lvinfo['max'] - $lvinfo['min'];
			$lv_len = $ispoper['level_point'] - $lvinfo['min'];
	...
	public function level_info($levels) {
		global $ispop, $levelset, $levelinfo, $theme;
		if($levels) {
			list($gnu_level,$ispop_level,$anonymous) = explode('|',$levels);
			if($anonymous == 'y') {
				$level['anonymous'] = true;
				return $level;
			} else {
				$level['gnu_name'] = $levelset['gnu_alias_'.$gnu_level];
				$level['name'] = $levelinfo[$ispop_level]['name'];
				$level['gnu_level'] = $gnu_level;
				$level['ispop_level'] = $ispop_level;

				$icon_path = EYOOM_THEME_PATH.'/'.$theme.'/image/level_icon';
				$icon_dir = EYOOM_THEME_URL.'/'.$theme.'/image/level_icon';
				if($ispop['use_level_icon_gnu'] == 'y') {
					if($gnu_level == 10) $_gnu_level = 'admin';
					else $_gnu_level = $gnu_level;
					$gnu_path = $icon_path.'/gnuboard/'.$ispop['level_icon_gnu'].'/'.$_gnu_level.'.gif';
					if(file_exists($gnu_path)) $level['gnu_icon'] = $icon_dir.'/gnuboard/'.$ispop['level_icon_gnu'].'/'.$_gnu_level.'.gif';
				}
				if($ispop['use_level_icon_ispop'] == 'y') {
					if($gnu_level == 10) $_ispop_level = 'admin';
					else $_ispop_level = $ispop_level;
					$ispop_path = $icon_path.'/ispop/'.$ispop['level_icon_ispop'].'/'.$_ispop_level.'.gif';
					if(file_exists($ispop_path)) {
						$level['ispop_icon'] = $icon_dir.'/ispop/'.$ispop['level_icon_ispop'].'/'.$_ispop_level.'.gif';
						$level['grade_icon'] = $icon_dir.'/grade/'.$ispop['level_icon_ispop'].'/g'.$_ispop_level.'.gif';
					}
				}
				return $level;
			}
	...
	public function point_comment() {
		global $g5, $member, $ispop_board, $cmt_amt, $board, $wr_id, $comment_id, $wr;

		unset($point);
		// 첫댓글 포인트
		if($ispop_board['bo_firstcmt_point'] > 0 && !$cmt_amt && $member['mb_id'] != $wr['mb_id']) {
			$point['firstcmt'] = $ispop_board['bo_firstcmt_point_type'] == 1 ? $this->random_num($ispop_board['bo_firstcmt_point']-1)+1 : $ispop_board['bo_firstcmt_point'];
			if($ispop_board['bo_cmtpoint_target'] == '1') {
			...
			} else if($ispop_board['bo_cmtpoint_target'] == '2') {
				$this->level_point($point['firstcmt']);
			}
		}

		// 지뢰폭탄 포인트 - 게시판 여유필드 wr_2를 사용
		if($ispop_board['bo_bomb_point'] > 0 && $ispop_board['bo_bomb_point_limit'] > 0 && $ispop_board['bo_bomb_point_cnt'] > 0 && $wr['wr_2']) {
			$bomb = @unserialize($wr['wr_2']);
			if(is_array($bomb)) {
				foreach($bomb as $key => $val) {
					if($val == $cmt_amt) {
						$point['bomb'][$key] = $ispop_board['bo_bomb_point_type'] == 1 ? $this->random_num($ispop_board['bo_bomb_point']-1)+1 : $ispop_board['bo_bomb_point'];
						if($ispop_board['bo_cmtpoint_target'] == '1') {
		...
		// 럭키 포인트
		if($ispop_board['bo_lucky_point'] > 0 && $ispop_board['bo_lucky_point_ratio'] > 0) {
			$max = ceil(100/$ispop_board['bo_lucky_point_ratio']);
			$random = $this->random_num($max-1);
			if($random%$max == 0) {
				$point['lucky'] = $ispop_board['bo_lucky_point_type'] == 1 ? $this->random_num($ispop_board['bo_lucky_point']-1)+1 : $ispop_board['bo_lucky_point'];
				if($ispop_board['bo_cmtpoint_target'] == '1') {
	...
	// 내용 가공
	public function ispop_content($content) {
	...
	private function video_from_soruce($src) {
		...
		$info = $this->ispop_host($video_url);
		$host = $info['host'];
		$query = $info['query'];
		$video['host'] = $host;
	...
	public function short_url_data($t) {
		...
		if(!$s_no || !is_int($s_no)) {
			...
			$link = sql_fetch("select * from {$g5['ispop_link']} where s_no = '{$s_no}' limit 1", false);
			if($link) {
				if (isset($link['wr_id'])) {
	...
	public function get_short_url() {
		...
		if(!$bo_table || !$wr_id || !$theme) {
			...
			$link = sql_fetch("select * from {$g5['ispop_link']} where bo_table = '{$bo_table}' and wr_id = '{$wr_id}' and theme = '{$theme}' ", false);
			if($link['bo_table']) {
	...
	public function make_short_url() {
		global $g5, $bo_table, $wr_id, $theme;
		$sql = "insert into {$g5['ispop_link']} set bo_table='{$bo_table}', wr_id = '{$wr_id}', theme = '{$theme}'";
		sql_query($sql,false);
