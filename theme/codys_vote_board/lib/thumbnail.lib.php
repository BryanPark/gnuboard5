...
function get_list_thumbnail($bo_table, $wr_id, $thumb_width, $thumb_height, $is_create=false, $is_crop=true, $crop_mode='center', $is_sharpen=false, $um_value='80/0.5/3')
{
    ...
    $row = sql_fetch($sql);
	//ChromePhp::log("row['bf_file'] is : " . $row['bf_file']);
    ...
    if($row['bf_file']) {
		...
		//ChromePhp::log("alt : " . $alt);
		//ChromePhp::log(" 일로 들어오나?ㅇㅇ");
    } else {
...
{
	//ChromePhp::log("thumbnail(); 여기 들어오니?");
    global $g5;

	//ChromePhp::log("thumb_width랑 height가 없는걸로 밝혀짐");
