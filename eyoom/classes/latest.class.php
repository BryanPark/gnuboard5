...
class latest extends eyoom
{
		...
		for($i=0; $row = sql_fetch_array($result); $i++) {
			...
			if(!$row['wr_subject']) {
				...
				if(preg_match('/secret/',$row['wr_option']) && (($is_member && !$is_admin && $member['mb_id']!=$row['mb_id']) || !$is_member)) {
					...
					$list[$i]['wr_subject'] = cut_str(strip_tags(htmlspecialchars_decode($row['wr_subject'])), $cut_subject, '…');
	...
	//수정 by Shin 최신글 리스트 추출
	public function latestList($option){
		$where = 1;
		$opt = $this->get_option($option);
		$where .= $opt['where'];
		$where .= "and wr_id = wr_parent";
	...
	//수정 by Shin[이윰 최신글(이슈토론) 추출]
	public function latest_issue_list($option){
		$list = $this->latestList($option);
		$liData;

		foreach($list as $key => $value){
			$liData .= "
				<li class=\"list-group-item dash\" style=\"font-size:12px;\"><a href=".$value['href'].">".$value['wr_subject']."</a></li>
			";
		}
	...
	//수정 by Shin 게시판 통합 최근글 추출
	public function latest_community_list($option){
		global $g5;
		...
		if(count($list) > 0){
			foreach($list as $key => $value){
				$sql = "select me_name from {$g5['eyoom_menu']} where me_pid = '{$value['bo_table']}'";
				$result = sql_query($sql);
				$row = sql_fetch_array($result);

				$liData .="
					<li class=\"list-group-item dash\">[".$row['me_name']."]&nbsp;<a href=".$value['href'].">".$value['wr_subject']."</a></li>
				";
			}
		}
		...
		if(count($list) < 9){
			...
			for($i=0; $i<$count; $i++){
				$liData .= "
					<li class=\"list-group-item dash\">글이 등록되지 않았습니다.</li>
				";
			}
		}
	...
	//공지사항 출력
	public function latest_get_notice(){
		global $g5;

		$board_table = $g5['write_prefix']."notice";

		$sql = "select* from {$board_table} order by wr_datetime desc limit 1";
		$res = sql_query($sql);
		...
		$resData = "
			<h1 style=\"font-weight:normal; margin:0; padding=10px 0 0 20px;\"></h1>
		";
	}
}
?>