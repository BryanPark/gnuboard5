<?php
	...
	//월별폴더 생성
	@mkdir(G5_DATA_PATH.'/file/'.$bo_table.'/'.date('Ym'), G5_DIR_PERMISSION);
	@chmod(G5_DATA_PATH.'/file/'.$bo_table.'/'.date('Ym'), G5_DIR_PERMISSION);
	...
	for ($i=0; $i<count($_FILES['cmt_file']['name']); $i++) {
		...
		if (is_uploaded_file($tmp_file)) {
			...
			$upload[$i]['file'] = date('Ym').'/'.abs(ip2long($_SERVER['REMOTE_ADDR'])).'_'.substr($shuffle,0,8).'_'.str_replace('%', '', urlencode(str_replace(' ', '_', $filename)));
