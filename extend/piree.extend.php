...
###########################################################
	...
	#########################################################
			...
			//===================================================
			// 레벨_관련__프로그램_번호
			define('PIREE_PLUS_REPORT_PROG_N',	'770006');


			//===================================================
			// 디렉토리
			define('PIREE_PLUS_REPORT_DIR',		'p'.PIREE_PLUS_REPORT_PROG_N.'__report');
			define('PIREE_PLUS_REPORT_URL',		PIREE_URL.'/'.PIREE_PLUS_REPORT_DIR);
			define('PIREE_PLUS_REPORT_PATH',	PIREE_PATH.'/'.PIREE_PLUS_REPORT_DIR);
			...
			# 시작 => 신고하기__링크
			#####################################################
			function get_report_do_url($prog_div_c, $bo_table, $wr_id)
			{
					...
					SWITCH ($prog_div_c)
					{
							...
							CASE 'article' :
									...
									// 테이블_코드__게시글_번호__없으면
									IF (!$bo_table)									$in_error++;
									IF (!$wr_id)										$in_error++;

									// 신고_링크
									$report_url = PIREE_PLUS_REPORT_URL."/do.pop.php?prog_div_c=".$prog_div_c."&bo_table=".$bo_table."&wr_id=".$wr_id;
							...
							CASE 'member' :
									...
									// 신고_링크
									$report_url = PIREE_PLUS_REPORT_URL."/do.pop.php?prog_div_c=".$prog_div_c."&mb_id=".$wr_id;
			...
			# PWP__770006__REPORT
			#####################################################
			//function get__is_blind($wr_pi_report_t) //2016-07-07 by Bryan Park  - 수정 - 블라인드 여부를 $wr_blind로 파악하게.
			function get__is_blind($list_arr) 
			{
				...
				//=================================================
				// 블라인드__여부
				$blind_arr["user_ck"]	= false;
				$blind_arr["admin_ck"] = false;
				$blind_arr["clear_ck"] = false;
				$blind_arr["report_t"] = $list_arr['wr_pi_report_t'];
				$blind_arr["wr_blind"] = $list_arr['wr_blind'];
				$wr_blind = $blind_arr["wr_blind"];

				//=================================================
				// 시작 => 블라인드__이면
				//IF ($wr_pi_report_t < 100) //2016-07-07 by Bryan Park  - 수정 $wr_blind == 0일때로.
				IF ($wr_blind == 0 )
				{
				...
				//ELSE IF ($wr_pi_report_t < 200) //2016-07-07 by Bryan Park  - 수정 $wr_blind == 1일때로.
				ELSE IF ($wr_blind == 1 )
				{

					// 블라인드__처리
					$blind_arr["user_ck"] = true;

					// 신고건수
					//$blind_arr["report_t"] = $wr_pi_report_t-100; //2016-07-07 by Bryan Park -삭제 -신고건수는 그대로 둬도 됨.


					//===============================================
					// 시작 => 관리자__아니면
					IF (!is_admin($_SESSION['ss_mb_id']))
					{

						// 블라인드__처리
						$blind_arr["admin_ck"] = true;
				...
				ELSE
				{
					...
					// 신고건수
					$blind_arr["report_t"] = 0; ////2016-07-07 by Bryan Park - 주석 - $wr_blind == 2일때 -> 관리자가 복원시킨 게시물.
